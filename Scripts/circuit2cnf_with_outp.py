...
def create_and_solve_CNF(task_index, clauses, new_clauses, max_var, solver):
  ...
  return answer, solvetime, conflicts, model
...
def solve_CNF(cnf, solver):
  solver = subprocess.run(["../Apps/" + solver], capture_output=True, text=True, input = cnf)
  result = solver.stdout.split('\n')
  errors = solver.stderr
  #print(result)
  if len(errors) > 0:
...
if __name__ == '__main__':
  ...
  answer, solvetime, conflicts, model = create_and_solve_CNF(0, clauses_list, create_random_inp, max_var, 'kissat')
  ...
  # if LECfilename[0] is '.':
  #   sat_assignment_filename = '.' + LECfilename[1:].split('.')[0] + '.sat_assignment'
